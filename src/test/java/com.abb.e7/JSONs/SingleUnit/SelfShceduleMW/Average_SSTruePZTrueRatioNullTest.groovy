...
class Average_SSTruePZTrueRatioNullTest {
  ...
  public void post() {
    ...
    def pricePatternsFistBlock = ["^43\\.2(\\d+)", "^46\\.0(\\d+)", "^54\\.5(\\d+)", "^54\\.5(\\d+)"] as List<Pattern>
    def pricePatternsSecondBlock = ["0\\.0", "^54\\.5(\\d+)", "^54\\.5(\\d+)"]
    def pricePatternsThirdBlock = ["^54\\.5(\\d+)"]
