...
class Incremental_WithBidLibraryInTheMiddleTest {
  ...
  public void post() {
    ...
    def pricePatternsFistBlock = ["^48\\.2(\\d+)", "^49\\.6(\\d+)", "^52\\.3(\\d+)", "^52\\.3(\\d+)"] as List<Pattern>
    def pricePatternsSecondBlock = ["30\\.0"]
    def pricePatternsThirdBlock = ["^48\\.2(\\d+)", "^49\\.6(\\d+)", "^52\\.3(\\d+)", "^52\\.3(\\d+)"]
