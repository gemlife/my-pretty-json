...
class Incremental_SSTrueLastFirstBidTruePZTrueTest {
  ...
  public void post() {
    ...
    def pricePatternsFistBlock = ["0.0", "^79\\.5(\\d+)", "^83\\.4(\\d+)","^83\\.4(\\d+)"]as List<Pattern>
    def pricePatternsSecondBlock = ["0.0", "^79\\.5(\\d+)", "^83\\.4(\\d+)","^83\\.4(\\d+)"]
    def pricePatternsThirdBlock = ["0.0", "^79\\.5(\\d+)", "^83\\.4(\\d+)","^83\\.4(\\d+)"]
