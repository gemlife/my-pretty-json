...
class Average_FBAndLBTrueEMForStationMWhStartUpTrueOutageLastPeriodOnlyTest {
  ...
  public void post() {
    ...
    def pricePatternsFistBlock = ["^54\\.7(\\d+)", "^57\\.9(\\d+)", "^61\\.(\\d+)", "^70\\.6(\\d+)"] as List<Pattern>
    def pricePatternsSecondBlock = ["^54\\.7(\\d+)", "^57\\.9(\\d+)", "^61\\.(\\d+)", "^70\\.6(\\d+)"]
