...
class Exponential_WithBidLibraryInTheMiddleTest {
  ...
  public void post() {
    ...
    def pricePatternsFistBlock = ["^17\\.1(\\d+)", "^17\\.8(\\d+)", "^31\\.5(\\d+)", "^31\\.5(\\d+)"] as List<Pattern>
    def pricePatternsSecondBlock = ["30\\.0"]
    def pricePatternsThirdBlock = ["^17\\.1(\\d+)", "^17\\.8(\\d+)", "^31\\.5(\\d+)", "^31\\.5(\\d+)"]
